...
description: Verify ONE behavioural-breaking-change candidate end to end — author the driver test, run the 3-state differential, diagnose failures, and report the verdict. Use when running a candidate from an agent worklist (<toolkit>/agent/*.json). One candidate per invocation.
...
`<toolkit>/specs/bump_breaks_catalog.json` for `verify.signal_grep`,
...
**4. Read the ledger row, not just the exit code.** `<toolkit>/output/05_verify/_*_differential.jsonl`, last row
...
`<toolkit>` is the directory holding `agent/`, `mine/`, `specs/`, `output/` and
`verified_cases/` — historically `scripts/nadia_scripts/`, but the folder is self-contained and
may sit anywhere, including a repository root. Locate it once at the start (look for the
directory containing `specs/bump_breaks_catalog.json`) and use it for every path below. Do not
assume the old `scripts/nadia_scripts` prefix; every script inside resolves its own paths
relative to its file, so only YOUR shell commands need the right prefix.

Machine-specific JDK locations come from `BBC_JDK_8` / `BBC_JDK_11` / `BBC_JDK_17` /
`BBC_JDK_21` when set; otherwise `agent/orchestrator.py` scans the usual install roots for
each version. If one is missing, set the variable rather than editing the source —
`python <toolkit>/install.py --doctor` prints what resolved on this machine, along with the
Maven repo the differential will use.

> **Editing this file:** the master is `<toolkit>/portable/claude/agents/bbc-driver-author.md`,
> and `<repo>/.claude/agents/bbc-driver-author.md` is a copy that `install.py` writes — Claude
> Code only discovers agents at the repo root, but the toolkit has to carry its own agent to
> another repo. Edit the master and re-run `install.py`. `install.py --check` fails if the two
> have drifted.
